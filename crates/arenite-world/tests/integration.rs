...
use arenite_sim::{
    chunk::ChunkData,
    material::{flags, MaterialInstance},
...
fn integration_generate_and_tick() {
    ...
    for dx in -3..=3i32 {
        sim.set_pixel(TilePos::new(64 + dx, 10), water);
    }
    for _ in 0..100 {
        sim.tick_simulation();
    }
...
fn integration_worldgen_has_surface_tiles() {
    ...
    let found =
        (0..32i32).any(|y| sim.get_pixel(TilePos::new(32, y)).physics == PhysicsType::Solid);
...
fn integration_fire_spreads_to_flammable_wood() {
    ...
    let wood = MaterialInstance {
        id: 12,
        physics: PhysicsType::Solid,
        color: arenite_core::Color::WOOD,
        light: [0.0; 3],
