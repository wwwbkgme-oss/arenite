...
impl ItemBuilder {
    ...
    pub fn category(mut self, c: ItemCategory)  -> Self { self.category     = c; self }
    pub fn stack(mut self, n: u32)              -> Self { self.max_stack    = n; self }
    pub fn price(mut self, p: u32)              -> Self { self.price        = p; self }
    pub fn dig_radius(mut self, r: i32)         -> Self { self.dig_radius   = r; self }
    pub fn damage(mut self, d: u32)             -> Self { self.damage       = d; self }
    pub fn mining_speed(mut self, s: f32)       -> Self { self.mining_speed = s; self }
    pub fn material(mut self, k: impl Into<String>) -> Self {
        self.material_key = Some(k.into()); self
    }
    pub fn heal(mut self, h: u32) -> Self { self.heal_amount = h; self }
...
pub fn default_item_registry() -> ItemRegistry {
    ...
    macro_rules! placer {
        ($key:expr, $name:expr, $stack:expr) => {
            r.register($key, Item::builder($name)
                .category(TilePlacer)
                .material($key)
                .stack($stack)
                .price(1)
    ...
    r.register("wooden_pickaxe", Item::builder("Wooden Pickaxe")
        .desc("A basic pickaxe carved from wood.  Mines soft terrain.")
        .category(Tool).rarity(Common)
        .price(50).dig_radius(2).mining_speed(0.7)
        .build());

    r.register("iron_pickaxe", Item::builder("Iron Pickaxe")
        .desc("A sturdy iron pickaxe.  Mines most stone without issue.")
        .category(Tool).rarity(Uncommon)
        .price(300).dig_radius(3).mining_speed(1.0)
        .build());

    r.register("gold_pickaxe", Item::builder("Gold Pickaxe")
        .desc("Gleaming gold — faster but still not suitable for obsidian.")
        .category(Tool).rarity(Rare)
        .price(800).dig_radius(3).mining_speed(1.4)
        .build());

    r.register("titanium_pickaxe", Item::builder("Titanium Pickaxe")
        .desc("Space-age alloy.  Cuts through obsidian like butter.")
        .category(Tool).rarity(Epic)
        .price(2500).dig_radius(4).mining_speed(2.0)
        .build());

    r.register("diamond_drill", Item::builder("Diamond Drill")
        .desc("The pinnacle of mining technology — drills anything.")
        .category(Tool).rarity(Legendary)
        .price(8000).dig_radius(5).mining_speed(3.5)
        .build());

    // ── Weapons — swords ─────────────────────────────────────────────────

    r.register("wooden_sword", Item::builder("Wooden Sword")
        .desc("Better than nothing.  Barely.")
        .category(Weapon).rarity(Common)
        .price(30).damage(5)
        .build());

    r.register("iron_sword", Item::builder("Iron Sword")
        .desc("A reliable iron blade.  Standard soldier issue.")
        .category(Weapon).rarity(Uncommon)
        .price(250).damage(15)
        .build());

    r.register("gold_sword", Item::builder("Gold Sword")
        .desc("Gold is soft but this is surprisingly sharp.")
        .category(Weapon).rarity(Rare)
        .price(700).damage(22)
        .build());

    r.register("titanium_sword", Item::builder("Titanium Sword")
        .desc("Cuts through most armour with ease.")
        .category(Weapon).rarity(Epic)
        .price(2000).damage(40)
        .build());

    r.register("crystal_sword", Item::builder("Crystal Sword")
        .desc("Forged from deep crystal.  It hums faintly.")
        .category(Weapon).rarity(Legendary)
        .price(6000).damage(65)
        .build());

    // ── Consumables ───────────────────────────────────────────────────────

    r.register("health_potion", Item::builder("Health Potion")
        .desc("Restores 50 HP.  Tastes faintly of copper.")
        .category(Consumable).rarity(Common)
        .stack(20).price(25).heal(50)
        .build());

    r.register("mega_health_potion", Item::builder("Mega Health Potion")
        .desc("Restores 200 HP.  Don't drink too fast.")
        .category(Consumable).rarity(Rare)
        .stack(10).price(200).heal(200)
        .build());
...
impl ItemStack {
    pub fn new(item_key: impl Into<String>, count: u32) -> Self {
        Self { item_key: item_key.into(), count }
    }
}
