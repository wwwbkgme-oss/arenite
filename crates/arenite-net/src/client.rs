...
impl GameClient {
    ...
    pub async fn connect(
        ...
        // Send Hello.
        write_message(&mut stream, &NetMessage::Hello {
        ...
        let player_id = match read_message(&mut stream).await? {
            NetMessage::Welcome { player_id, world_seed, world_width, world_height, spawn_x, spawn_y } => {
                info!(
        ...
        tokio::spawn(async move {
            loop {
                ...
                match bincode::deserialize::<NetMessage>(&buf) {
                    Ok(msg) => { let _ = recv_tx.send(msg); }
                    Err(e)  => warn!("Deserialise error: {}", e),
                }
            }
        });

        Ok(Self { player_id, player_name, server_addr, send_tx, recv_rx })
    }
