...
fn bench_tick(c: &mut Criterion) {
    ...
    for n in [4i32, 8] {
        ...
        g.bench_with_input(BenchmarkId::new("tick_dynamic", n * n), &n, |b, _| {
            b.iter(|| sim.tick_simulation())
        });
    }
    let mut s = make_sim(8, false);
    g.bench_function("tick_all_static_64chunks", |b| {
        b.iter(|| s.tick_simulation())
    });
    g.finish();
}
